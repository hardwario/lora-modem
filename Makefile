...
ELF ?= $(BUILD_DIR)/$(TYPE)/$(BASENAME).elf
MAP ?= $(BUILD_DIR)/$(TYPE)/$(BASENAME).map
BIN ?= $(BUILD_DIR)/$(TYPE)/$(BASENAME).bin
HEX ?= $(BUILD_DIR)/$(TYPE)/$(BASENAME).hex
...
# generated by git describe is written to a file in $(BUILD_DIR) which is then
...
tmp := $(shell \
	...
	echo "$$cur" > $$f)
version := $(strip $(shell cat $(BUILD_DIR)/version))
...
OBJ_C = $(SRC_FILES:%.c=$(BUILD_DIR)/$(TYPE)/%.o)
OBJ_S = $(ASM_SOURCES:%.s=$(BUILD_DIR)/$(TYPE)/%.o)
OBJ = $(OBJ_C) $(OBJ_S)
DEP = $(OBJ:%.o=%.d)
...
$(BUILD_DIR)/$(TYPE)/src/%.o: src/%.c $(MAKEFILE_LIST) $(BUILD_DIR)/$(TYPE)/config
	$(call compile,\
		-I $(SRC_DIR) \
		-I $(SRC_DIR)/debug \
		-I $(CFG_DIR) \
...
$(BUILD_DIR)/$(TYPE)/lib/LoRaWAN/%.o: lib/LoRaWAN/%.c $(MAKEFILE_LIST) $(BUILD_DIR)/$(TYPE)/config
	$(call compile,\
		-I $(SRC_DIR) \
		-I $(CFG_DIR) \
		-isystem $(LIB_DIR)/stm/STM32L0xx_HAL_Driver/Inc \
...
$(BUILD_DIR)/$(TYPE)/src/cmd.o: CFLAGS+=-DVERSION='"$(version)"'
$(BUILD_DIR)/$(TYPE)/src/cmd.o: CFLAGS+=-DVERSION_COMPAT='"$(VERSION_COMPAT)"'
$(BUILD_DIR)/$(TYPE)/src/cmd.o: CFLAGS+=-DLIB_VERSION='"$(lib_version)"'
$(BUILD_DIR)/$(TYPE)/src/cmd.o: CFLAGS+=-DBUILD_DATE='"$(build_date)"'
$(BUILD_DIR)/$(TYPE)/src/cmd.o: CFLAGS+=-DBUILD_DATE_COMPAT='"$(BUILD_DATE_COMPAT)"'
$(BUILD_DIR)/$(TYPE)/src/cmd.o: $(MAKEFILE_LIST) $(BUILD_DIR)/$(TYPE)/config $(BUILD_DIR)/version $(BUILD_DIR)/lib_version

$(BUILD_DIR)/$(TYPE)/src/main.o: CFLAGS+=-DVERSION='"$(version)"'
$(BUILD_DIR)/$(TYPE)/src/main.o: CFLAGS+=-DLIB_VERSION='"$(lib_version)"'
$(BUILD_DIR)/$(TYPE)/src/main.o: CFLAGS+=-DBUILD_DATE='"$(build_date)"'
$(BUILD_DIR)/$(TYPE)/src/main.o: $(MAKEFILE_LIST) $(BUILD_DIR)/$(TYPE)/config $(BUILD_DIR)/version $(BUILD_DIR)/lib_version

$(BUILD_DIR)/$(TYPE)/lib/stm/%.o: lib/stm/%.c $(MAKEFILE_LIST) $(BUILD_DIR)/$(TYPE)/config
	$(call compile,\
...
$(BUILD_DIR)/$(TYPE)/lib/rtt/%.o: lib/rtt/%.c $(MAKEFILE_LIST) $(BUILD_DIR)/$(TYPE)/config
	$(call compile)

$(BUILD_DIR)/$(TYPE)/lib/loramac-node/%.o: lib/loramac-node/%.c $(MAKEFILE_LIST) $(BUILD_DIR)/$(TYPE)/config
	$(call compile,\
		-Wno-int-conversion \
		-Wno-unused-parameter \
		-Wno-switch-default \
...
$(BUILD_DIR)/$(TYPE)/cfg/%.o: cfg/%.c $(MAKEFILE_LIST) $(BUILD_DIR)/$(TYPE)/config
	$(call compile,-isystem $(LIB_DIR)/stm/STM32L0xx_HAL_Driver/Inc)

$(BUILD_DIR)/$(TYPE)/%.o: %.s $(MAKEFILE_LIST) $(BUILD_DIR)/$(TYPE)/config
	$(Q)$(ECHO) "Compiling: $<"
	$(Q)mkdir -p $(@D)
	$(Q)$(CC) -c $(ASFLAGS) $< -o $@

VERSION: $(MAKEFILE_LIST) $(BUILD_DIR)/version
	$(Q)cp $(BUILD_DIR)/version VERSION

LIB_VERSION: $(MAKEFILE_LIST) $(BUILD_DIR)/lib_version
	$(Q)cp $(BUILD_DIR)/lib_version LIB_VERSION
...
.PHONY: .clean-build
.clean-build: $(MAKEFILE_LIST)
	$(Q)$(ECHO) "Deleting build files..."
	$(Q)rm -rf "$(BUILD_DIR)"
